...
const Generator = {
    ...
    TorqueProgram(robotName) {
        ...
        for(let i=1; i<csvLines.length; i++) {
             ...
             if(mName && robotName.toLowerCase().includes(mName.toLowerCase()) || mName.toLowerCase().includes(robotName.toLowerCase())) {
                 ratios[0] = parseFloat(cols[5]) || 1.0; 
                 // Note: Original C# looped collecting ratios up to 6. Let's just use 1.0 if not fully provided.
                 break;
             }
        }
        return `ProgramInfo\n    Version = "S4.24"\n    VRC = "V4R24"\n    Time = "${TemplateHelper.getNowAmPm()}"\n    RobotName = "${robotName}"\nEndProgramInfo\nStart;\n    D_J1_torque = Abs(GetTorque(1) * ${ratios[0].toFixed(3)});\n    D_J2_torque = Abs(GetTorque(2) * ${ratios[1].toFixed(3)});\n    D_J3_torque = Abs(GetTorque(3) * ${ratios[2].toFixed(3)});\n    D_J4_torque = Abs(GetTorque(4) * ${ratios[3].toFixed(3)});\n    D_J5_torque = Abs(GetTorque(5) * ${ratios[4].toFixed(3)});\n    D_J6_torque = Abs(GetTorque(6) * ${ratios[5].toFixed(3)});\n    #================================================================================\n    ywCur_J1_torque = D_J1_torque;\n    ywCur_J2_torque = D_J2_torque;\n    ywCur_J3_torque = D_J3_torque;\n    ywCur_J4_torque = D_J4_torque;\n    ywCur_J5_torque = D_J5_torque;\n    ywCur_J6_torque = D_J6_torque;\nEnd;`;
    },
    ...
    LabelsJson(steps, options) {
        ...
        if (options.EnableTorque) {
            dVars.push({nIndex: 1, sLabel: "D_J1_torque", sDescription: "", sOriginalName: "D[1]"}, {nIndex: 2, sLabel: "D_J2_torque", sDescription: "", sOriginalName: "D[2]"}, {nIndex: 3, sLabel: "D_J3_torque", sDescription: "", sOriginalName: "D[3]"}, {nIndex: 4, sLabel: "D_J4_torque", sDescription: "", sOriginalName: "D[4]"}, {nIndex: 5, sLabel: "D_J5_torque", sDescription: "", sOriginalName: "D[5]"}, {nIndex: 6, sLabel: "D_J6_torque", sDescription: "", sOriginalName: "D[6]"});
        }
